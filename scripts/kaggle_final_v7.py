"""
Gold 1-Minute: FINAL Training V7 — All Models, All Tricks
===========================================================
Goal: Maximize accuracy on complete 6.7M dataset
Strategies:
  - H=20 and H=60 targets
  - SMOTE balancing
  - Confidence filtering (55%, 60%, 65%, 70%)
  - Stacking ensemble
  - Advanced features (Ichimoku, CCI, Stochastic, Williams %R, OBV)
  - Retry on failure
"""
...
CHECKPOINT_FILE = '/kaggle/working/checkpoint_v7.json'
...
print("=" * 70)
print("FINAL V7: ALL MODELS, ALL TRICKS")
print("=" * 70)
...
if cp.get('data_loaded'):
    print("\n[SKIP] Data loaded")
    df = pd.read_pickle('/kaggle/working/df_v7.pkl')
else:
    print("\n[1/7] Loading data...")
    ...
    df.to_pickle('/kaggle/working/df_v7.pkl')
    cp['data_loaded'] = True
...
if cp.get('features_done'):
    print("\n[SKIP] Features done")
    F = pd.read_pickle('/kaggle/working/features_v7.pkl')
else:
    ...
    # Targets (H=20 and H=60)
    for H in [20, 60]:
        ret = close.pct_change(H).shift(-H)
        F[f'target_ret_{H}'] = ret
        rv = ret.rolling(1000).std()
        F[f'target_vol_{H}'] = np.where(abs(ret) > 0.5 * rv, np.sign(ret), 0)
    ...
    F.to_pickle('/kaggle/working/features_v7.pkl')
...
for HORIZON in [20, 60]:
    ...
    target_col = f'target_vol_{HORIZON}'
    feature_cols = [c for c in F.columns if not c.startswith('target_')]
    mask = F[target_col] != 0
    ...
    for name, probs in [('LGB', lgb_probs), ('XGB', xgb_probs), ('RF', rf_probs)]:
        ...
        for thresh in [0.55, 0.60, 0.65, 0.70]:
            ...
            if conf_mask.sum() > 100:
                ...
                conf_acc = accuracy_score(y_test[conf_mask], conf_preds)
    ...
    for thresh in [0.55, 0.60, 0.65, 0.70]:
        ...
        if conf_mask.sum() > 100:
            conf_preds = (ens_probs[conf_mask] > 0.5).astype(int)
            conf_acc = accuracy_score(y_test[conf_mask], conf_preds)
            conf_pct = conf_mask.mean()
        else:
            conf_acc = 0; conf_pct = 0
    ...
    joblib.dump({'lgb': lgb_model, 'xgb': xgb_model, 'rf': rf_model, 'features': selected},
                f'/kaggle/working/models_h{HORIZON}.pkl')
...
results_df.to_csv('/kaggle/working/final_results_v7.csv', index=False)

# Best by confidence accuracy
best = results_df.loc[results_df['conf_acc'].idxmax()]
print("\n" + "=" * 70)
print("BEST CONFIGURATION")
print("=" * 70)
print(f"  Model: {best['model']} | Horizon: {best['horizon']}")
print(f"  Confidence Threshold: {best['conf_thresh']}")
print(f"  Confident Accuracy: {best['conf_acc']:.4f} ({best['conf_acc']*100:.2f}%)")
print(f"  Confident Coverage: {best['conf_pct']:.2%}")
print(f"  Overall Accuracy: {best['overall_acc']:.4f}")
print(f"  Overall AUC: {best['overall_auc']:.4f}")

print("\n" + "=" * 70)
print("TOP 10 BY CONFIDENT ACCURACY")
print("=" * 70)
for _, row in results_df.sort_values('conf_acc', ascending=False).head(10).iterrows():
    print(f"  {row['model']:10} H={row['horizon']:2} thresh={row['conf_thresh']:.2f}  "
          f"conf_acc={row['conf_acc']:.4f}  coverage={row['conf_pct']:.2%}")
