...
## Corpus Check
- 46 files · ~116,110 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1711 nodes · 4300 edges · 47 communities detected
...
- `update_rl_signal()` --calls--> `fetch_yahoo_finance_data()`  [INFERRED]
  pages/smma_strategy.py → app.py
- `execute_auto_trade()` --calls--> `fetch_yahoo_finance_data()`  [INFERRED]
  pages/smma_strategy.py → app.py
- `update_smma_strategy()` --calls--> `fetch_yahoo_finance_data()`  [INFERRED]
  pages/smma_strategy.py → app.py
- `_fetch_recent_data()` --calls--> `fetch_yahoo_finance_data()`  [INFERRED]
  pages/precision_strategy.py → app.py
- `_fetch_training_data()` --calls--> `fetch_yahoo_finance_data()`  [INFERRED]
  pages/precision_strategy.py → app.py
...
Cohesion: 0.02
Nodes (179): reset(), Backtester, Run Monte Carlo simulation on trade sequence.                  Parameters, Professional Backtesting Engine          Features:     - Vectorized and event-dr, Initialize backtester.                  Parameters         ----------         in, ExnessMT5Bridge, OrderType, PaperTradingBridge (+171 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (89): create_volatility_model(), ModelParameters, Goldman Sachs-Level Volatility Modeling Framework ==============================, Compute GARCH(p, q) log-likelihood.                  Parameters         --------, Fit GARCH(p, q) model using Maximum Likelihood Estimation.                  Para, Compute the full conditional variance series., Compute AIC, BIC, and other fit statistics., Compute EGARCH(1,1) log-likelihood.                  EGARCH captures leverage ef (+81 more)
...
Cohesion: 0.11
Nodes (132): generate_fallback_data(), _lazy_load_models(), _load_rl_state_from_disk(), Professional Trading Terminal - Dash Frontend ==================================, Calculate probability of profit using Black-Scholes framework.          Returns, Create advanced metrics cards - Advanced Tab., Create risk metrics cards - Risk Tab., Generate trading signals from real technical indicators. (+124 more)

### Community 4 - "Community 4"
Cohesion: 0.03
Nodes (66): _build_fold_table(), _empty_wf_fig(), _fetch_recent_data(), _fetch_training_data(), _format_pf(), _get_system(), Precision Strategy Page — 1-minute multi-asset trading system ==================, Fetch recent 1-min OHLCV data via app.py's existing helper. (+58 more)

### Community 5 - "Community 5"
Cohesion: 0.03
Nodes (110): _analyze_impact(), _analyze_sentiment(), calculate_adx_safe(), calculate_atr_wilder(), calculate_bb_safe(), calculate_bollinger_bands(), calculate_bs_probability(), calculate_cci_safe() (+102 more)
...
Cohesion: 0.03
Nodes (62): calculate_calmar_ratio(), calculate_var(), calibrate(), calibrate_from_market(), call_price(), d1(), d2(), delta() (+54 more)
...
Cohesion: 0.06
Nodes (33): analyze(), analyze_batch(), analyze_market(), analyze_sentiment(), analyze_sentiment_batch(), chat_with_ai(), get_ai_status(), get_local_ai_service() (+25 more)
...
Cohesion: 0.08
Nodes (16): DeepQNetwork, DeepRLAgent, Load agent state from disk. Returns True on success., Deep Q-Network for trading., Deep Reinforcement Learning Agent using DQN with Experience Replay., Initialize Deep RL agent.              Parameters             ----------, Get action using epsilon-greedy policy., Store transition in replay buffer. (+8 more)
...
## Knowledge Gaps
- **546 isolated node(s):** `================================================================================`, `Per-asset configuration with research-backed parameters.`, `Layer 0: Fundamental and macro context integration.      Components:     - Econo`, `Identify trading session regime.`, `Compute news impact score (0-1, higher = more dangerous).          Args:` (+541 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 22`** (2 nodes): `__init__.py`, `Page Registry for Multi-Page Dash Application`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 23`** (1 nodes): `Align higher timeframe data to lower timeframe index.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 24`** (1 nodes): `Compute Average True Range.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 25`** (1 nodes): `Check if FinBERT can be loaded.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 26`** (1 nodes): `Load the financial-sentiment model singleton.          Tries each entry in MODEL`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 27`** (1 nodes): `Analyze sentiment of financial text.                  Returns:             Dict`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 28`** (1 nodes): `Fallback keyword-based sentiment analysis.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 29`** (1 nodes): `Analyze multiple texts.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 30`** (1 nodes): `Absorption: large volume with minimal price movement (institutional accumulation`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 31`** (1 nodes): `Reindex a higher-TF close series onto the 1m index via forward-fill.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 32`** (1 nodes): `Add mtf_score and mtf_confluence to a 1m DataFrame.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 33`** (1 nodes): `Simulate intra-bar fill realism using OHLC ordering by direction.          For L`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 34`** (1 nodes): `Expected Calibration Error (lower is better; <0.05 = well calibrated).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 35`** (1 nodes): `Calculate d1 parameter.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 36`** (1 nodes): `Calculate d2 parameter.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 37`** (1 nodes): `Calculate European call option price.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 38`** (1 nodes): `Calculate European put option price.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 39`** (1 nodes): `Calculate option delta.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 40`** (1 nodes): `Calculate option gamma.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 41`** (1 nodes): `Calculate option vega.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 42`** (1 nodes): `Calculate option theta.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 43`** (1 nodes): `Calculate option rho.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 44`** (1 nodes): `Calculate implied volatility using Newton-Raphson method.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 45`** (1 nodes): `Calibrate SABR parameters to market volatilities using L-BFGS-B.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 46`** (1 nodes): `Calibrate SABR from typical market data.                  Can work with limited`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Action` connect `Community 3` to `Community 2`, `Community 12`?**
  _High betweenness centrality (0.114) - this node is a cross-community bridge._
- **Why does `NewsSource` connect `Community 3` to `Community 2`, `Community 6`?**
  _High betweenness centrality (0.109) - this node is a cross-community bridge._
- **Why does `VolatilityModels` connect `Community 1` to `Community 10`, `Community 3`, `Community 5`?**
